...
document.getElementById("toggle-menu").addEventListener("click", (e) => {
  ...
  let nav_bar = document
    ...
    .classList.add("toggle-nav-bar--show");
});
...
function check_row_social() {
  ...
  if (
    home_section.getBoundingClientRect().bottom <
    social_row_home.getBoundingClientRect().bottom
  ) {
...
//**************read more **********************/
document
  .querySelector("section.qualifications .read-more button")
  .addEventListener("click", (e) => {
    document.querySelector("section.qualifications  .read-more").style.display =
      "none";
    ...
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        ...
        let active_menu = document.querySelector(
          `.toggle-nav-bar [data-name="${entry_id}"]`
        );
        document
          .querySelector(`.toggle-nav-bar li.active`)
        ...
        els.forEach((el) => {
          el.classList.forEach((className) => {
            if (
              className.startsWith("effect-") &&
              !className.endsWith("__active")
            ) {
...
for (let i = 0; i < signature_paths.length; i++) {
  ...
  signature_paths[i].style.setProperty(
    "--length",
    signature_paths[i].getTotalLength()
  );
}
// ***************send email*****************************
  ...
  emailjs
    ...
    .then(
      ...
      () => {
        // if not sended
        message_for_submit.innerHTML =
          "something is wrong!<br>please try again later";
        message_dialog_box.showModal();
      }
    );
};
